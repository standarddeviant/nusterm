...
impl App {
    ...
    fn submit_message(&mut self) {
        // TODO: leverage ANSI colors???
        self.messages.push(format!("[sent='{}']", self.input.clone()));
        self.input.clear();
        self.reset_cursor();
    }
}

pub fn user_input_main() -> Result<(), Box<dyn Error>> {
    // setup terminal
    enable_raw_mode()?;
...
fn ui(f: &mut Frame, app: &App) {
    ...
    let [messages_area, help_area, input_area, ] = vertical.areas(f.area());
